...
import me.rainny.reaper.factionutils.event.CaptureZoneEnterEvent;
import me.rainny.reaper.factionutils.event.CaptureZoneLeaveEvent;
...
import me.rainny.reaper.factionutils.type.*;
...
public class ProtectionListener implements Listener {
            ...
            if (event.useTravelAgent() && to.getWorld().getEnvironment() == World.Environment.NORMAL) {
                ...
                if (factionAt instanceof ClaimableFaction) {
                    ...
                    if (playerFaction != factionAt) {
                        player.sendMessage(ChatColor.GRAY + "Portal would have created portal in territory of " + factionAt.getDisplayName(player) + ChatColor.GRAY + '.');
                        event.setCancelled(true);
                    }
                }
            }
            ...
            if (attacker != null) {
                ...
                if (playerFaction != null && ((attackerFaction = plugin.getFactionManager().getPlayerFaction(attacker)) != null)) {
                    ...
                    if (attackerFaction == playerFaction) {
                        ...
                        event.setCancelled(true);
						attacker.sendMessage(SettingsYML.ALLY_COLOUR + hiddenAstrixedName + ChatColor.GRAY + " is an ally.");
                    }
                }
            }
        }
        ...
        if (entered instanceof Player) {
            ...
            if (vehicle instanceof Horse) {
                ...
                if (owner != null && !owner.getUniqueId().equals(entered.getUniqueId())) {
                    ((Player) entered).sendMessage(ChatColor.GRAY + "You cannot ride a Horse that belongs to " + ChatColor.RED + owner.getName() + ChatColor.GRAY + '.');
                    event.setCancelled(true);
                }
            }
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.PHYSICAL) { // Prevent players from trampling on crops or pressure plates, etc.
            ...
            if (!canBuild && !attemptBuild(event.getPlayer(), block.getLocation(), ChatColor.GRAY + "You cannot do this in the territory of %1$s" + ChatColor.GRAY + '.', true)) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (HCF.getPlugin().getFactionManager().getFactionAt(event.getPlayer().getLocation()) == HCF.getPlugin().getFactionManager().getFaction("GlowstoneMountain")){
         ...
         if (!attemptBuild(event.getPlayer(), event.getBlock().getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
          event.setCancelled(true);
        }
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getBlockPlaced().getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onBucketFill(PlayerBucketFillEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getBlockClicked().getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onHangingBreakByEntity(HangingBreakByEntityEvent event) {
        ...
        if (remover instanceof Player) {
            if (!attemptBuild(remover, event.getEntity().getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onHangingPlace(HangingPlaceEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getEntity().getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onHangingDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (entity instanceof Hanging) {
            ...
            if (!attemptBuild(attacker, entity.getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onHangingInteractByPlayer(PlayerInteractEntityEvent event) {
        ...
        if (entity instanceof Hanging) {
            if (!attemptBuild(event.getPlayer(), entity.getLocation(), ChatColor.GRAY + "You may not build in the territory of %1$s" + ChatColor.GRAY + '.')) {
                event.setCancelled(true);
            }
        }
    }
        ...
        if (result) {
            ...
            if (!isInteraction && factionAt instanceof WarzoneFaction) {
                if (denyMessage != null && player != null) {
                    player.sendMessage(ChatColor.GRAY + "You may not build in the " + factionAt.getDisplayName(player) + ChatColor.GRAY + ".");
                }
