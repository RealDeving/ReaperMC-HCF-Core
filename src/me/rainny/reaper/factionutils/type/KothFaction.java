...
public class KothFaction extends CapturableFaction implements ConfigurationSerializable {
    ...
    public void printDetails(CommandSender sender) {
        ...
        for (Claim claim : claims) {
            ...
            sender.sendMessage(ChatColor.GRAY + "  Location: " + ChatColor.RED + '(' + ENVIRONMENT_MAPPINGS.get(location.getWorld().getEnvironment()) + ", " + location.getBlockX() + " | "
                    + location.getBlockZ() + ')');
        }
        ...
        if (captureZone != null) {
            ...
            if (remainingCaptureMillis > 0L && remainingCaptureMillis != defaultCaptureMillis) {
                sender.sendMessage(ChatColor.GRAY + "  Remaining Time: " + ChatColor.RED + DurationFormatUtils.formatDurationWords(remainingCaptureMillis, true, true));
            }

            sender.sendMessage(ChatColor.GRAY + "  Capture Delay: " + ChatColor.RED + captureZone.getDefaultCaptureWords());
            if (CaptureZone.getCappingPlayer() != null && sender.hasPermission("hcf.koth.checkcapper")) {
                ...
                sender.sendMessage(ChatColor.GRAY + "  Current Capper: " + ChatColor.RED + capping.getName() + ChatColor.GOLD + factionTag);
            }
        }
