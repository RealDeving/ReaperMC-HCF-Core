...
public class FactionChatSpyArgument extends CommandArgument implements Listener {
  ...
  public void onFactionChat(FactionChatEvent event)
  {
    ...
    String format = ChatColor.GOLD + "[" + ChatColor.RED + event.getChatChannel().getDisplayName() + ": " + ChatColor.GRAY + faction.getName() + ChatColor.GOLD + "] " + ChatColor.GRAY + event.getFactionMember().getRole().getAstrix() + player.getName() + ": " + ChatColor.GRAY + event.getMessage();
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("clear")) {
            currentSpies.clear();
            sender.sendMessage(ChatColor.GRAY + "You are no longer spying the chat of any faction.");
            return true;
        }
