package me.rainny.reaper.factionutils.claim;

import com.doctordark.utils.ItemBuilder;
import com.doctordark.utils.other.cuboid.Cuboid;
import com.doctordark.utils.other.cuboid.CuboidDirection;
...
public class ClaimHandler {
    ...
    public static final ItemStack SUBCLAIM_WAND = new ItemBuilder(Material.GOLD_SPADE, 1)
            ...
            .lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Set the first and second position of ", ChatColor.GRAY + "your Subclaim selection.", "", 
            		ChatColor.AQUA + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Clear your current Subclaim selection.", "", ChatColor.AQUA + "Use " + ChatColor.GRAY + 
            		"/faction subclaim create <name>" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Acquire your selected Subclaim.").build();
    ...
    public static final ItemStack CLAIM_WAND = new ItemBuilder(Material.WOOD_HOE)
            ...
            .lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Set the first and second position of ", 
            		ChatColor.GRAY + "your Claim selection.", "", ChatColor.AQUA + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.AQUA + " to:", ChatColor.GRAY + 
            		"Clear your current Claim selection.", "", ChatColor.GRAY + "Shift " + ChatColor.AQUA + "Left Click " + ChatColor.GREEN + "the Air or a Block" + ChatColor.AQUA + " to:", 
    ...
    public boolean tryPurchasing(Player player, Claim claim) {
        ...
        if (claimPrice > factionBalance) {
            player.sendMessage(ChatColor.RED + "You have " + EconomyManager.ECONOMY_SYMBOL + factionBalance + ", The price of this claim is " + EconomyManager.ECONOMY_SYMBOL
                    + claimPrice + '.');
        ...
        if (claim.getWidth() < MIN_CLAIM_RADIUS || claim.getLength() < MIN_CLAIM_RADIUS) {
            player.sendMessage(ChatColor.RED + "Claims must be atleast " + MIN_CLAIM_RADIUS + 'x' + MIN_CLAIM_RADIUS + " blocks.");
            return false;
        }
        ...
        for (int x = minimumX; x < maximumX; x++) {
            for (int z = minimumZ; z < maximumZ; z++) {
                ...
                if (factionAt != null && !(factionAt instanceof WildernessFaction)) {
                    player.sendMessage(ChatColor.RED + "This claim is not in " + ChatColor.GRAY + "Wilderness" + ChatColor.RED + '.');
                    return false;
                }
            }
        }
        ...
        player.sendMessage(ChatColor.AQUA + "You have purchased the claim for " + ChatColor.GREEN + EconomyManager.ECONOMY_SYMBOL + claimPrice + ChatColor.AQUA + '.');
        playerFaction.setBalance(factionBalance - claimPrice);
        playerFaction.broadcast(ChatColor.GOLD + player.getName() + ChatColor.GREEN + " claimed some land for your faction at " + ChatColor.GOLD + '(' + center.getBlockX() + ", " + center.getBlockZ()
