...
import me.rainny.reaper.timer.event.TimerStartEvent;
...
public class CombatTimer extends PlayerTimer implements Listener {
    ...
    public void onTimerStart(TimerStartEvent event) {
        if (event.getTimer() == this) {
            ...
            if (optional.isPresent()) {
                Player player = optional.get();
                player.sendMessage(ChatColor.GRAY + "You are now Spawn Tagged for " + ChatColor.RED
                        + DurationFormatUtils.formatDurationWords(event.getDuration(), true, true) + ChatColor.GRAY + '.');
            }
        }
    }
